...
async function withDept(rows: (typeof staffTable.$inferSelect)[]) {
  ...
  return rows.map((r) => ({ ...iso(r), departmentName: r.departmentId ? (dMap.get(r.departmentId) ?? null) : null }));
}
...
router.get("/staff", async (req, res): Promise<void> => {
  ...
  const rows = await db.select().from(staffTable).where(filters.length ? and(...filters) : undefined).orderBy(desc(staffTable.createdAt));
...
router.patch("/staff/:id", async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [row] = await db.update(staffTable).set(parsed.data).where(eq(staffTable.id, params.data.id)).returning();
...
router.delete("/staff/:id", async (req, res): Promise<void> => {
  ...
  if (!params.success) { res.status(400).json({ error: params.error.message }); return; }
  const [row] = await db.delete(staffTable).where(eq(staffTable.id, params.data.id)).returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
  res.sendStatus(204);
});
